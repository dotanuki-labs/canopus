...
fn should_detect_owner_syntax_issue() {
    ...
    execution
        .failure()
        .stderr(contains("L1 : cannot parse owner (codeowners-syntax)"));
}
...
fn should_detect_glob_syntax_issue() {
    ...
    execution
        .failure()
        .stderr(contains("L1 : invalid glob pattern (codeowners-syntax)"));
}

#[test]
fn should_collate_issues_on_the_same_entry() {
    let codeowners = indoc! {"
        [z-a]*.rs    org/crabbers
    "};
    ...
    let expected = indoc! {"
        L1 : invalid glob pattern (codeowners-syntax)
        L1 : cannot parse owner (codeowners-syntax)
    "};

    execution.failure().stderr(contains(expected));
}
